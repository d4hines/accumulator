let assert_equal_poly (type a) (message: string) (left: a) (right: a) =
 ...
 let call_as_exn (type parameter) (caller : address) (params : parameter) (amount : tez) (taddr : (parameter, storage) typed_address) : storage =
...
let call_as (type parameter) (caller : address) (params : parameter) (amount : tez) (taddr : (parameter, storage) typed_address) : test_exec_result =
    let contract = Test.to_contract taddr in
    let () = Test.set_source caller in
    Test.transfer_to_contract contract params amount

let call_as_and_expect (type parameter) (expect : string)
